...
struct ComputeOutput {
    ...
    __host__ __device__ void operator()(Index ValidCellIndex)
    {
        ...
        if (m_data.IsUnstructured()) {
            ...
            const auto &cl = &m_data.Cl()[Cellbegin];
            const auto &unstrType = m_data.Tl()[CellNr] & ~UnstructuredGrid::CONVEX_BIT;

            switch (unstrType) {
            case UnstructuredGrid::HEXAHEDRON:
                m_data.interpolUnstr(CellNr, unstrType, ValidCellIndex, 0, hexaTriTable, hexaEdgeTable);
                break;
            case UnstructuredGrid::TETRAHEDRON:
                m_data.interpolUnstr(CellNr, unstrType, ValidCellIndex, 0, tetraTriTable, tetraEdgeTable);
                break;
            case UnstructuredGrid::PYRAMID:
                m_data.interpolUnstr(CellNr, unstrType, ValidCellIndex, 0, pyrTriTable, pyrEdgeTable);
                break;
            case UnstructuredGrid::PRISM:
                m_data.interpolUnstr(CellNr, unstrType, ValidCellIndex, 0, prismTriTable, prismEdgeTable);
                break;
